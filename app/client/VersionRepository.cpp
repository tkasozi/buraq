...
UpdateInfo VersionRepository::main_version_logic()
{
	...
	try
	{
		...
		if (std::stoi(ver[0]) > APP_VERSION_MAJOR || std::stoi(ver[1]) > APP_VERSION_MINOR || std::stoi(ver[2]) > APP_VERSION_PATCH)
		{
...
{
	return std::to_string(APP_VERSION_MAJOR) + "." + std::to_string(APP_VERSION_MINOR) + "." + std::to_string(APP_VERSION_PATCH);
};
...
std::filesystem::path VersionRepository::downloadNewVersion() const
{
	...
	std::filesystem::path latestRelease = std::filesystem::temp_directory_path() / "ITools" / ("it-tools-" + versionInfo.latestVersion + ".zip");
